...
function qidiriv(barchaOquvchilar) {
    const qidiruvNatija = document.getElementById('natija');
    const kantaynerDetali = document.getElementById('details');

     qidiruvNatija.innerHTML = '';
    kantaynerDetali.style.display = 'none';

       if (barchaOquvchilar.trim() === '') {
        return;
    }

    const saralabOlish = students.filter(student =>
        student.name.toLowerCase().includes(barchaOquvchilar.toLowerCase())
    );

     saralabOlish.forEach(student => {
        const resultItem = document.createElement('div');
        resultItem.className = 'result-item';
        resultItem.textContent = student.name;
        resultItem.onclick = () => elementlarniKorsatish(student);
        qidiruvNatija.appendChild(resultItem);
    });
}

function elementlarniKorsatish(student) {
    const kantaynerDetali = document.getElementById('details');
    kantaynerDetali.style.display = 'block';
    kantaynerDetali.innerHTML = `
        <h3>${student.name}</h3>
        <p><strong>Foydalanuvchi nomi:</strong> ${student.username}</p>
        <p><strong>Email:</strong> ${student.email}</p>
        <p><strong>Yosh:</strong> ${student.age}</p>
        <p><strong>Baho:</strong> ${student.grade}</p>
        <p><strong>Mutaxassislik:</strong> ${student.major}</p>
        <button onclick="closeDetails()">Yopish</button>
    `;
}
...
document.addEventListener('click', function(event) {
    ...
    const natija = document.getElementById('natija');
     ...
     if (!kantaynerDetali.contains(event.target) && !natija.contains(event.target)) {
        closeDetails();
    }
});
